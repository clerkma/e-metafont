...
make_string (void) 
{
  register str_number Result; if (strptr == maxstrptr) 
  {
...
void 
zprintpath (halfword h , str_number s , boolean nuline) 
{
  /* 30 31 */ halfword p, q  ;
...
void 
zprintedges (str_number s , boolean nuline , integer xoff , integer yoff) 
{
  halfword p, q, r  ;
  integer n  ;
...
void 
zprintpen (halfword p , str_number s , boolean nuline) 
{
...
void 
zdisperr (halfword p , str_number s) 
{
  if (interaction == 3) 
  ;
  print_nl (765) ;
...
zpplusq (halfword p , halfword q , smallnumber t) 
{
  ...
  lab30: mem[p + 1].cint = slow_add (mem[p + 1].cint , mem[q + 1]
  .cint) ;
  mem[r].hhfield .v.RH = p ;
  depfinal = p ;
  Result = mem[memtop - 1].hhfield .v.RH ;
...
expand (void) 
{
  ...
  case 8 : 
	...
	backinput () ;
	if ((str_start[curexp + 1]- str_start[curexp]) > 0) 
	{
	  beginfilereading () ;
	  curinput .namefield = 2 ;
	  k = first + (str_start[curexp + 1]- str_start[curexp]) ;
	  if (k >= maxbufstack) 
	  {
	    if (k >= bufsize) 
	    {
...
endname (void) 
{
  ...
  curarea = 261 ;
    ...
    curarea = strptr ;
    incr (strptr) ;
    str_start[strptr] = areadelimiter + 1 ;
  } 
  if (extdelimiter == 0) 
  curext = 261 ;
  else {
...
zpackfilename (str_number n , str_number a , str_number e) 
{
  ...
  if (nameoffile) 
  libcfree (nameoffile) ;
  nameoffile = xmallocarray (ASCII_code , (str_start[a + 1]- str_start[a 
...
zpackbufferedname (smallnumber n , integer a , integer b) 
{
  ...
  if (nameoffile) 
  libcfree (nameoffile) ;
  nameoffile = xmallocarray (ASCII_code , n + (b - a + 1) + 6) ;
...
makenamestring (void) 
{
  ...
  Result = 63 ;
    ...
    for_end) do 
      {
	str_pool[poolptr] = xord[nameoffile[k]];
	incr (poolptr) ;
      } 
    while (k++ < for_end) ;} 
    Result = make_string () ;
...
zstrtonum (quarterword c) 
{
  ...
  if (c == 49) {
    ...
    if ((str_start[curexp + 1]- str_start[curexp]) == 0) 
    n = -1 ;
    else n = str_pool[str_start[curexp]];
  } 
  else {
    ...
    n = 0 ;
    badchar = false ;
    {register integer for_end; k = str_start[curexp];for_end = str_start[
...
zdounary (quarterword c) 
{
  ...
  case 50 : 
    ...
    badunary (50) ;
      ...
      if (curexp < 0) 
      curexp = curexp + 256 ;
      if ((str_start[curexp + 1]- str_start[curexp]) != 1) 
      {
	{
	  if (poolptr + 1 > maxpoolptr) 
	  {
  ...
  case 51 : 
    if (curtype == 4) 
    flushcurexp ((str_start[curexp + 1]- str_start[curexp]) * 65536L) 
    ;
    else if (curtype == 9) 
    ...
    flushcurexp (pyth_add (mem[mem[curexp + 1].cint + 1].cint , mem[
...
zcat (halfword p) 
{
  ...
  b = curexp ;
  {
    if (poolptr + (str_start[a + 1]- str_start[a]) + (str_start[b + 
    1]- str_start[b]) > maxpoolptr) 
    {
      if (poolptr + (str_start[a + 1]- str_start[a]) + (str_start[b 
      + 1]- str_start[b]) > poolsize) 
      overflow (257 , poolsize - initpoolptr) ;
      maxpoolptr = poolptr + (str_start[a + 1]- str_start[a]) + (
...
zdobinary (halfword p , quarterword c) 
{
  ...
  case 82 : 
	...
	goto lab30 ;
      ...
      flushcurexp (str_vs_str (mem[p + 1].cint , curexp)) ;
  ...
  case 74 : 
    if ((curtype == 16) && (mem[p].hhfield .b0 == 16)) {
      ...
      if (c == 73) 
      curexp = pyth_add (mem[p + 1].cint , curexp) ;
      else curexp = pyth_sub (mem[p + 1].cint , curexp) ;
    } 
    else badbinary (p , c) ;
    break ;
  case 84 : 
...
initgf (void) 
{
  ...
  selector = oldsetting ;
  {
    gfbuf[gfptr] = (poolptr - str_start[strptr]) ;
    incr (gfptr) ;
    if (gfptr == gflimit) 
    gfswap () ;
  } 
...
doshowstats (void) 
{
  ...
  print_ln () ;
  print_nl (952) ;
  print_int (strptr - initstrptr) ;
  print_char (38) ;
  print_int (poolptr - initpoolptr) ;
  print (558) ;
  print_int (maxstrings - maxstrptr) ;
  print_char (38) ;
  print_int (poolsize - maxpoolptr) ;
  print (951) ;
  print_ln () ;
  getxnext () ;
} 
